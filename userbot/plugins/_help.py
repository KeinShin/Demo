...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "𝔅𝔩𝔞𝔠𝔨 𝔏𝔦𝔤𝔥𝔱𝔫𝔦𝔫𝔤User"
CMD_HNDLR = Var.CMD_HNDLR
CUSTOM_HELP_EMOJI = os.environ.get("CUSTOM_HELP_EMOJI", "𝔅𝔩")
...
async def cmd_list(event):
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        ...
        if tgbotusername is None or input_str == "text":
            ...
            help_string = f"""`Userbot Helper for {DEFAULTUSER} to reveal all the commands of `**[𝔅𝔩𝔞𝔠𝔨 𝔏𝔦𝔤𝔥𝔱𝔫𝔦𝔫𝔤](https://xditya.gitbook.io/telebot/)**\n\n"""
            try:
